...
export class GroupsApi {
  ...
  static async searchUsers(
    ...
    const response = await fetch(
      `${this.baseUrl}/search-users?${searchParams}`,
    );
  ...
  static async createScoreRecord(formData: {
    groupId: string;
    ruleId: string;
    targetUserId: string;
    criteria?: any;
  ...
  static async updateScoreRecord(
    id: string,
    formData: { points?: number; notes?: string; recordedAt?: Date },
  ): Promise<any> {
    const response = await fetch("/api/score-records", {
      method: "PUT",
      headers: {
  ...
  static async getScoringRules(groupId?: string): Promise<any[]> {
    const url = groupId
      ? `/api/scoring-rules?groupId=${groupId}`
      : "/api/scoring-rules";
  ...
  static async createScoringRule(formData: {
    name: string;
    description?: string;
    criteria: any;
    points: number;
    autoAddToGroups?: boolean;
  }): Promise<any> {
    const response = await fetch("/api/scoring-rules", {
      method: "POST",
      headers: {
    ...
    if (data.autoAddToGroups && data.groupsAdded > 0) {
      toast.success(
        `Quy tắc "${data.scoringRule.name}" đã được tạo và tự động thêm vào ${data.groupsAdded} nhóm!`,
      );
    }
  ...
  static async removeRuleFromGroup(
    groupId: string,
    ruleId: string,
  ): Promise<void> {
    const response = await fetch(
      `/api/groups/${groupId}/rules?groupId=${groupId}&ruleId=${ruleId}`,
      {
        method: "DELETE",
      },
    );
  ...
  static async updateScoringRule(
    id: string,
    formData: {
      name: string;
      description?: string;
