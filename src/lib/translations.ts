...
export const SCORE_RECORDING = {
  ...
  NO_ACTIVE_RULES: 'Không có quy tắc chấm điểm đang hoạt động. Liên hệ quản trị viên để thêm quy tắc.',
  BADGE_POINTS: '{points} điểm',
  OVERRIDE_CHECKBOX: 'Ghi đè điểm mặc định ({points} điểm)',
  LABEL_POINTS: 'Điểm *',
  UNIT_POINTS: 'điểm',
  WARNING_DIFFERENT: 'ℹ️ Điểm tùy chỉnh: {customPoints} điểm (ghi đè điểm mặc định: {defaultPoints})',
  LABEL_DATE: 'Ngày *',
  ...
  MESSAGES: {
    ...
    CONFIRM_OVERRIDE: 'Bạn đang ghi {customPoints} điểm nhưng quy tắc "{ruleName}" thường cho {defaultPoints} điểm. Tiếp tục?',
  },
}
...
export const ADMIN_USERS = {
  ...
  DIALOGS: {
    DELETE: {
      ...
      DESCRIPTION: 'Bạn có chắc chắn muốn xoá {name}? Hành động này không thể hoàn tác.',
      BUTTON_CANCEL: 'Huỷ',
      BUTTON_DELETE: 'Xoá',
    },
    UPDATE_ROLE: {
    ...
    RESET_PASSWORD: {
      ...
      TOGGLE_SHOW: 'Hiện',
      TOGGLE_HIDE: 'Ẩn',
      PASSWORD_REQUIREMENTS: 'Phải có ít nhất 8 ký tự bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt.',
      BUTTON_CANCEL: 'Huỷ',
...
export const API = {
  ERROR: {
    ...
    PASSWORD_COMMON: 'Mật khẩu này quá phổ biến. Vui lòng chọn mật khẩu an toàn hơn.',
    ...
    RULE_INACTIVE: 'Không tìm thấy quy tắc chấm điểm hoặc quy tắc đang bị vô hiệu',
    RULE_NOT_BELONG_TO_GROUP: 'Quy tắc chấm điểm này không thuộc về nhóm',
    CANNOT_DELETE_OWNER: 'Không thể xóa chủ nhóm',
    CANNOT_DELETE_GROUP_WITH_DATA: 'Không thể xóa nhóm khi còn bản ghi điểm hoặc quy tắc chấm điểm',
    ...
    ONLY_ADMIN_OWNER_MANAGE_SCORES: 'Chỉ quản trị viên hoặc chủ nhóm mới có thể ghi điểm',
    ONLY_ADMIN_OWNER_EDIT_SCORES: 'Chỉ quản trị viên hoặc chủ nhóm mới có thể chỉnh sửa bản ghi điểm',
    ONLY_ADMIN_OWNER_DELETE_SCORES: 'Chỉ quản trị viên hoặc chủ nhóm mới có thể xóa bản ghi điểm',
    ONLY_ADMIN_OWNER_MANAGE_RULES: 'Chỉ quản trị viên hoặc chủ nhóm mới có thể tạo quy tắc cho nhóm này',
    ONLY_ADMIN_DELETE_RULE: 'Chỉ quản trị viên hệ thống mới có thể xóa quy tắc chấm điểm',
    ONLY_ADMIN_UPDATE_RULE: 'Chỉ quản trị viên hệ thống mới có thể cập nhật quy tắc chấm điểm',
    CANNOT_DELETE_RULE_WITH_RECORDS: 'Không thể xóa quy tắc vì còn bản ghi điểm sử dụng quy tắc này',
    CANNOT_DELETE_RULE_IN_USE_BY_GROUPS: 'Không thể xóa quy tắc vì đang được sử dụng bởi các nhóm: {groupNames}',
    ...
    NEED_GROUP_ID_RULE_ID_TARGET: 'Cần cung cấp mã nhóm, mã quy tắc và mã người nhận điểm',
  ...
  SUCCESS: {
    ...
    MEMBER_ROLE_UPDATED: 'Đã cập nhật vai trò của {email} thành {role} trong nhóm "{groupName}"',
    MEMBER_REMOVED: 'Đã xóa {email} khỏi nhóm "{groupName}"',
    OWNERSHIP_TRANSFERRED: 'Đã chuyển quyền sở hữu "{groupName}" từ {fromEmail} sang {toEmail}',
    RULE_CREATED: 'Đã tạo quy tắc chấm điểm "{ruleName}" cho nhóm "{groupName}"',
    RULE_CREATED_GLOBAL: 'Đã tạo quy tắc chấm điểm toàn cục "{ruleName}" với {points} điểm',
    ...
    SCORE_RECORDED: 'Đã ghi {points} điểm cho {userName} bằng quy tắc "{ruleName}"',
    SCORE_UPDATED: 'Đã cập nhật bản ghi điểm cho {userName}: {points} điểm',
    SCORE_DELETED: 'Đã xóa bản ghi điểm của {userName}: {points} điểm bằng quy tắc "{ruleName}"',
