  ...
  const validateForm = () => {
      ...
      if (overridePoints) {
        ...
        if (selectedRule && points !== selectedRule.points) {
          ...
          const confirmMessage = SCORE_RECORDING.MESSAGES.CONFIRM_OVERRIDE
            .replace('{customPoints}', points.toString())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newScoreRecord = await GroupsApi.createScoreRecord(scoreData)
            ...
            const successMessage = SCORE_RECORDING.MESSAGES.SUCCESS.replace('{points}', newScoreRecord.points.toString())
              ...
              <Label htmlFor="member-select" className="text-base font-medium flex items-center gap-2">
                            ...
                            >
                              <SelectTrigger className="mt-2">
                                <SelectValue placeholder={SCORE_RECORDING.PLACEHOLDER_MEMBER} />
                              </SelectTrigger>
                              <SelectContent>
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="rule-select" className="text-base font-medium">
                                ...
                                >
                                  <SelectTrigger className="mt-2">
                                    <SelectValue placeholder={SCORE_RECORDING.PLACEHOLDER_RULE} />
                                  </SelectTrigger>
                                  <SelectContent>
                  ...
                  <CardContent className="pt-4">
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant="outline" className="text-primary">
                                              ...
                                              {SCORE_RECORDING.BADGE_POINTS.replace('{points}', selectedRule.points.toString())}
                                            </Badge>
                                          </div>
                                          ...
                                          <div className="flex items-center space-x-2 mt-4 pt-4 border-t">
                                            ...
                                            >
                                              {SCORE_RECORDING.OVERRIDE_CHECKBOX.replace('{points}', selectedRule.points.toString())}
                                            </Label>
                                          </div>
                                        </CardContent>
                                      </Card>
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="relative">
                                        ...
                                        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                          <span className="text-sm text-muted-foreground">{SCORE_RECORDING.UNIT_POINTS}</span>
                                        </div>
                                      </div>
                                      {parseFloat(formData.points) !== selectedRule.points && formData.points && (
                                        <p className="text-sm text-blue-600">
                                          {SCORE_RECORDING.WARNING_DIFFERENT
                                            .replace('{defaultPoints}', selectedRule.points.toString())
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    />
                                    <p className="text-sm text-muted-foreground">
                                      {SCORE_RECORDING.CHARACTER_COUNT.replace('{count}', formData.notes.length.toString())}
                                    </p>
                                  </div>
