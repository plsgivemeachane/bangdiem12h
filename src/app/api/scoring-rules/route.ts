...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { id, name, description, criteria, points, isActive } =
      await request.json();
    ...
    await logActivity({
      ...
      description: API.SUCCESS.RULE_UPDATED.replace(
        "{ruleName}",
        updatedRule.name,
      ),
      metadata: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, description, criteria, points, groupId, autoAddToGroups } =
      await request.json();
    ...
    if (groupId) {
      ...
      await logActivity({
        ...
        description: API.SUCCESS.RULE_CREATED.replace(
          "{ruleName}",
          scoringRule.name,
      ...
      try {
        ...
        const eligibleGroups = groups.filter(
          (group) =>
            ...
            ["OWNER", "ADMIN"].includes(group.members[0].role),
        );
    ...
    await logActivity({
      userId: session.user.id,
      action: ActivityType.SCORING_RULE_CREATED,
      description: API.SUCCESS.RULE_CREATED_GLOBAL.replace(
        "{ruleName}",
        scoringRule.name,
      ).replace("{points}", String(scoringRule.points)),
      metadata: {
        ruleName: scoringRule.name,
        points: scoringRule.points,
        criteria,
