  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/users/${passwordDialog.user.id}/password`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
                      ...
                      <div className="text-sm text-gray-600">
                        {userData._count && (
                          <div className="flex gap-2">
                            ...
                            <span title="Tham gia nhóm">
                              {userData._count.groupMemberships}m
                            </span>
                            <span title="Điểm">
                              {userData._count.scoreRecords}s
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2 justify-end">
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>
                              {userData.id === user?.id
                                ? "Không thể sửa vai trò của chính bạn"
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {deleteDialog.user?.name || deleteDialog.user?.email}? Hành động
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <div className="mt-2">
                <Badge
                  variant={
                    roleDialog.user?.role === "ADMIN" ? "default" : "secondary"
                  }
                >
                  {roleDialog.user
                    ? ROLE_LABELS[roleDialog.user.role] || roleDialog.user.role
                    : ""}
                </Badge>
              </div>
            </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {passwordDialog.user?.name || passwordDialog.user?.email}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Phải có ít nhất 8 ký tự bao gồm chữ hoa, chữ thường, số và ký tự
                đặc biệt.
              </p>
            </div>
          </div>
